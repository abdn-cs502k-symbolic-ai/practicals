...
\textbf{How the two hours run.}
The first hour is questions 1 to 5.
Question 1 is on paper and questions 2 to 5 run in the online editor at \url{http://editor.planning.domains}, so you need nothing installed to start.
The second hour is question 6 on its own: a warehouse described in prose, with no skeleton to fill in, written against the clock and handed in as the four files Assessment 1 asks for.
That is the rehearsal, and it is the part of tomorrow morning that this session exists to prepare.

\textbf{Questions 7 and 8 go home with you.}
Question 7 builds your own planner and validator, and question 8 is two extensions to the cup of tea domain.
Neither is optional in any sense that matters: both tutorials in week 13 open assuming you have a working Fast Downward, and Assessment 3 requires one you built and can configure yourself.
The expectation is that you arrive next week having either built it or brought the error to a demonstrator, and this session is the last one with demonstrators in the room before then.
...
That is correct for this problem, where the two rooms are joined, and wrong in general: see question 8(b).
...
The test that catches it is to ask for two deliveries, which is question 8(a).}
...
\item
\question{\textbf{This question is the second hour of the session, and it is the one that rehearses tomorrow morning.}
Everything above gave you a vocabulary: question 2 handed you the types and predicates and left the action bodies blank, and question 5 handed you a working domain to edit.
Assessment 1 hands you a paragraph and an empty file, and choosing the predicates is the part that takes the time.
So this question hands you a paragraph and an empty file.

\textbf{The warehouse.}
A warehouse is divided into bays, and some pairs of bays are joined by an aisle.
...
\textbf{What to hand in}, and note that these are the deliverables of Assessment 1, named the same way:
\begin{enumerate}
...
\textbf{Spend the hour roughly like this.}
...
Nobody finishes comfortably, which is the point: Assessment 1 is also timed.

\textbf{Two checks you can run without any tools installed.}
\begin{enumerate}
\setcounter{enumii}{4}
...
If your planner brings back a plan that moves both at once, you have the bug from question 3 and you have found it yourself.
...
\end{enumerate}
No numeric fluents.
Assessment 1 forbids them and so does this question: the battery is symbolic, and working out how to make it symbolic is most of the exercise.}
\answer{Two encodings follow. The first is the one most people reach, and it is pure STRIPS. The second is what ADL buys you, and it is the shape the Assessment 1 reference solution itself uses. Both were run under Fast Downward and validated with VAL on 2026-09-25.

\textbf{What this question is really marking.}
Not the action bodies, which follow mechanically once the predicates are right, but the four decisions taken before any of them: that a bay is an object and an aisle is a fact about a pair of them, that carrying is a relation between a forklift and a pallet with a companion predicate saying the forks are empty, that a battery level is an \emph{object} rather than a number, and that the order of the levels is itself data the problem file supplies.
A student who gets those four right writes the rest in ten minutes. A student who does not is still fighting the domain when the hour ends.

\textbf{The symbolic battery, encoding one: levels as objects.}
\pddlfile{solutions/warehouse/domain.pddl}
The whole trick is \texttt{(next ?above ?below)}.
The levels are objects like any other, and the fact that full sits above half sits above low sits above empty is three facts in the initial state, not anything the domain knows.
\texttt{drive} then asks for a level below the current one and moves to it, and it cannot fire at empty because no \texttt{next} fact matches.
\texttt{recharge} is the same action with the two levels the other way round.
Nothing here counts, and that is what makes it STRIPS: the planner is matching facts, not doing arithmetic.

\textbf{The reference instance}, at the scale floor the question asks for:
\pddlfile{solutions/warehouse/problem1.pddl}
Note why this instance makes the battery bite.
...
An instance whose bays are all adjacent to a charger would parse, solve, and rehearse nothing.
Designing the instance is part of the modelling, which is the lesson Assessment 3 Stage 1 will ask you for again.
Fast Downward returns a 30-step plan for it, of which seven steps are recharges.

\textbf{Encoding two: conditional effects.}
...
This is shorter, it needs \texttt{:adl} in the requirements, and it produces shorter plans: 27 steps on the same instance, of which four are recharges, because one recharge action buys a full battery.
Either encoding is correct. Being able to say why you picked one is what the modelling note is for.

\textbf{The two-pallet check.}
\pddlfile{solutions/warehouse/problem2.pddl}
Eight actions if the domain is right: pick up, drive, put down, drive back, recharge, pick up, drive, put down.
A \texttt{pick-up} missing \texttt{(not (free ?f))} carries both pallets at once and returns something shorter.
This is question 3's lesson arriving a second time, except that this time the student built the instance that exposes it, which is the habit worth leaving the room with.

\textbf{The unsolvable instance.}
...
Fast Downward explores the state space and reports no solution, which is a different thing from a planner that has not finished yet, and students should see the difference once.

\textbf{What goes wrong, in the order it goes wrong.}
Forgetting \texttt{(not (free ?f))} on pick-up, which the two-pallet instance catches.
Writing \texttt{connected} in one direction only and then wondering why the forklift can reach the dock and not come back: an aisle is two facts, exactly as the doorway in question 8 is.
Declaring \texttt{:requirements :adl} and using only STRIPS, or the reverse, which the translator rejects as firmly as question 5(f) does.
And putting the battery in a numeric fluent, which the online editor will happily parse and Assessment 1 forbids.}

%% SOURCE: Ours, 2026-09-16, made take-home 2026-09-25 when the session was split into an hour of questions and an hour of modelling. A build that goes wrong must not eat either hour, so it goes home, with the expectation stated plainly and the demonstrators available during question 6 for anyone who finishes early.
\item
\question{\textbf{Take this home, and treat it as expected rather than optional.}

Everything in this session ran in the online editor.
From here on you need your own planner: both tutorials in week 13 open assuming one, Assessment 3 requires a planner you built and can configure yourself, and the editor will not do for either.
...
\item Run it on your finished cup of tea domain from question 2. Does it return the plan you expected?
\item Get VAL, either the prebuilt binary or from source, and use it to check the plan you validated by hand in question 4. Does it agree with you?
\item Then run VAL on your warehouse domain from question 6, against the two-pallet instance you wrote for it. This is the one that finds a missing delete effect, and it is worth doing before the domain matters.
\end{enumerate}

We expect you to arrive at the week 13 tutorial with this working.
If a build defeats you, that is ordinary and the guide's troubleshooting section covers the two failures that account for most of it, but do not let it sit: bring the error to a demonstrator.
Today's session is the last one with demonstrators in the room before week 13, so if you finish question 6 early, start the build while there is someone to ask.}
...
\textbf{Fast Downward stops with an \texttt{AssertionError} before it looks at your domain.}
That is the requirements line, and it is question 5(f): the translator checks what you \emph{declared} rather than what you used, so a requirement you listed and never needed stops the build as firmly as one you need and it lacks.

\textbf{VAL says \texttt{Bad plan description!} and nothing else.}
