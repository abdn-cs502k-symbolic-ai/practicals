...
\textbf{This session exercises knowledge for both PDDL assessments.}
You will sit Assessment 1 tomorrow  under invigilation. 
It consists of you to transcribing a given specification into PDDL.
...
You will use the first hour for questions 1 to 5.
Question 1 is an essay question, whereas questions 2 to 5 require you to run the online editor at \url{http://editor.planning.domains}, so you need nothing installed to start.
You will use the second hour for question 6 on its own.
It gives you a warehouse in prose and no skeleton, and you will deliver the same structure of four components Assessment 1 will require tomorrow. We strongly recommend that you do this against the clock, so you will practice for assessment.

\textbf{You will answer Questions 7 and 8 at home.}
Question 7 asks you to build your own planner and validator, and question 8 asks you to formalise two extensions to the cup of tea domain.
Both tutorials in week 13 assume a working Fast Downward, and Assessment 3 requires one you built and can configure yourself.
...
We know perfectly well that a language model can write every domain below, correctly, in seconds.
Assessment 1 is invigilated, and you will have no model available when you sit it. 
This, while using an LLM to do this for you can give you the illusion you of understanding, the only way you will be prepared is having written PDDL yourself, with the errors and the confusion that go with it.
...
Question 2 handed you the types and predicates and left the action bodies blank, and question 5 handed you a working domain to edit.
...
Assessment 1 asks for the same four files under the same names.
\begin{enumerate}
...
\textbf{What this question marks.}
The action bodies follow mechanically once the predicates are right, so the marks are in four decisions that come before them.
...
\textbf{Encoding one, levels as objects.}
...
\textbf{The reference instance.}
This one meets the scale floor the question asks for.
...
Question 3 made the same point with a broken model we supplied. Here the student writes the instance that exposes it.
...
\textbf{What usually goes wrong.}
Forgetting \texttt{(not (free ?f))} on pick-up, which the two-pallet instance catches.
Writing \texttt{connected} in one direction only, so that the forklift reaches the dock and cannot come back; an aisle is two facts, as the doorway in question 8 is.
Declaring \texttt{:requirements :adl} while using only STRIPS, or the reverse, which the translator rejects, as in question 5(f).
...
\item
\question{\textbf{Take this home. We expect it working before week 13.}
...
\item Then run VAL on your warehouse domain from question 6, against the two-pallet instance you wrote for it. That is the check that finds a missing delete effect.
\end{enumerate}
...
If that does not get you past it, bring the error to a demonstrator rather than leaving it until week 13.
Today is the last session with demonstrators in the room, so if you finish question 6 early, start the build while there is someone to ask.}
...
That is the requirements line, and question 5(f) covers it.
