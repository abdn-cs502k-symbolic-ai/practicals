...
\lstdefinelanguage{PDDL}{
  ...
  morekeywords=[2]{:domain,:requirements,:types,:constants,:predicates,:functions,:action,:parameters,:precondition,:effect,:objects,:init,:goal,:metric,:duration,:derived},
  morekeywords=[3]{:strips,:typing,:adl,:equality,:negative-preconditions,:disjunctive-preconditions,:conditional-effects,:universal-preconditions,:quantified-preconditions,:existential-preconditions,:fluents,:action-costs},
  sensitive=false,
  morecomment=[l]{;},
  moredelim=[s][\color{RedOrange}]{?}{\ },
}
%% HDDL is PDDL plus the hierarchy, so it inherits the definition above and adds
%% the constructs Lecture 10 introduces, in their own colour. \pddlfile selects it
%% on the .hddl extension, so a file cannot be printed in the wrong language.
\lstdefinelanguage{HDDL}[]{PDDL}{
  morekeywords=[3]{:hierarchy,:method-preconditions,:typeof-predicate},
  morekeywords=[4]{:task,:tasks,:method,:htn,:subtasks,:ordered-subtasks,:ordering},
}
\lstdefinestyle{pddl}{
  language=PDDL,
  ...
  keywordstyle=[4]\bfseries\color{Violet},
...
\newsavebox{\pddlbox}
\newcommand{\pddlfilebox}[2][]{\par\SuspendTagging{pddl}\begin{lrbox}{\pddlbox}\begin{minipage}{\linewidth}\lstinputlisting[style=pddl,#1]{#2}\end{minipage}\end{lrbox}\ResumeTagging{pddl}\noindent\usebox{\pddlbox}\par}
\ExplSyntaxOn
\NewDocumentCommand{\pddlfile}{O{}m}{
  \str_if_in:nnTF {#2} {.hddl} { \pddlfilebox[language=HDDL,#1]{#2} } { \pddlfilebox[#1]{#2} }
}
\ExplSyntaxOff
...
\pddlfile[firstline=6,lastline=26]{solutions/gripper-typed.pddl}
Now there are exactly 36 ground actions, the same 36 that could ever be applicable.

The key advantages of typing are twofold. 
The planner never builds the other 1052, so grounding is cheaper, and the parser catches a typing error in your domain rather than showing up as a problem the planner reports unsolvable.
The parser assistance is the key advantage because it catches a mistyped parameter at compile-time, whereas missing a type predicate becomes a silent bug.

However, typing does not guarantee correctness.
...
Lecture 8's planning graphs are the nearest equivalent you have seen: their layers do hold sets of non-mutex actions, but that is a relaxation used to compute a bound, not a schedule you can execute.

One more result points the other way.
...
\question{\textbf{Assessment Rehearsal.}
...
Assessment 1 hands you a paragraph and an empty file, and choosing the predicates takes most of the time, which we exercise in this question.
...
Prepare the four files below, which are the four Assessment 1 asks for, under the same names.
\begin{enumerate}
...
Like Assessment 1, this question forbids numeric fluents.
...
There are at least four common errors associated with this question.
...
\answer{There is nothing to mark here, but two failures recur and are worth knowing before they cost you an evening.
