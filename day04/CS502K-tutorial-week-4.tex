...
\textbf{You do not need anything installed to start.}
Question 1 is on paper and questions 2 to 5 run in the online editor at \url{http://editor.planning.domains}, which is the quicker way to begin.
Question 6 is the installation itself, and it is last on purpose: this session is one block rather than two, Assessment 1 having taken tomorrow morning, and a compile that goes wrong should cost you the end of the session rather than the start of it.
Do it before you leave, because everything in week 13 needs it and so does Assessment 3.
If you have already built Fast Downward and VAL, skip to question 6 whenever you want the validator for question 4.
...
Then, once you have built VAL in question 6, come back and check yourself against it:}
...
%% SOURCE: Ours, 2026-09-16. Last on purpose. Week 12 has one practical session rather than two, so a toolchain that will not build must not be allowed to consume the modelling half of it. Everything in week 13 and Assessment 3 depends on this working.
\item
\question{\textbf{Do this before you leave, whatever else you have finished.}

Everything so far ran in the online editor.
From here on you need your own planner: next week's tutorials both need one, Assessment 3 requires a planner you built and can configure yourself, and the editor will not do for either.
...
\item If either build fails, say so now rather than next week. Bring the error to a demonstrator while there is someone in the room.
\end{enumerate}}
