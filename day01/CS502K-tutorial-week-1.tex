...
\item This tutorial runs over both practical sessions of this week.
The questions below are pen and paper, and cover Lectures 1 and 2.

\item The second session uses the notebook \texttt{tutorial1-problem-formulation.ipynb} in this folder, which formulates the vacuum world of Lecture 2 as a search problem and sets up next week's search tutorial.
...
%% student you cannot give the answer, point them at Lectures 1 and 2 and at the relevant chapter
%% of Russell and Norvig, and offer instead to explain a concept or to criticise an answer they
...
\item 
%% SOURCE: AIMA 3e Ex 1.7.
%% Items (a) bar codes and (d) routing are AIMA's, with AIMA's answers.
%% The spam filter and the LLM are ours, added 2026-09-08; web search and voice menus are AIMA's, commented out.
...
%  \item Voice-activated telephone menus
  ...
  \item Although bar code scanning is in a sense computer vision, these are not AI systems.
...
%  \item To a limited extent. Such menus tends to use vocabularies which are very limited -- e.g. the digits, ``Yes'', and ``No'' -- and within the designers’ control, which greatly simplifies the problem. On the other hand, the programs must deal with an uncontrolled space of all kinds of voices and accents. The voice activated directory assistance programs used by telephone companies, which must deal with a large and changing vocabulary are certainly AI programs.
  \item This is borderline. There is something to be said for viewing these as intelligent agents working in cyberspace. 
  The task is sophisticated, the information available is partial, the techniques are heuristic (not guaranteed optimal), and the state of the world is dynamic.
  All of these are characteristic of intelligent activities. 
  On the other hand, the task is very far from those normally carried out in human cognition.
  \item Yes.
  The task is genuinely uncertain, an adversary shifts the distribution deliberately, and the filter learns its decision from data instead of applying a rule that somebody wrote down.
  A filter working from a fixed list of banned words fails that last test and would not qualify.
  The mechanism a system uses to reach its verdict settles the question here.
  \item Yes, on any of the four definitions in Section 1.1, though the interesting question is which one.
  It acts humanly, and acting humanly is what Turing proposed to measure.
  It does not think rationally in the sense this course follows, because no part of it manipulates sentences that stand for facts.
  It also holds no explicit goal and optimises no stated utility, so the ``acting rationally'' definition fits it only loosely.
  A system can be uncontroversially AI and still sit outside the strand we study.
  \end{enumerate}
}

\item
%% SOURCE: Ours, 2026-09-08, replacing AIMA 3e Ex 1.2 (commented out above).
%% Same source paper, but asks about the nine objections and about what the test measures, which 1.2 does not.
...
\begin{enumerate}
\item Which of the nine still hold, and which has the intervening 75 years settled?
\item Turing predicted that by 2000 an interrogator would stand no better than a 70\% chance of identifying the machine after five minutes of questioning.
Judged against a chatbot you have used, was he wrong about the date, about the difficulty, or about the test?
\item Passing the imitation game and thinking come apart in both directions.
Give one system that would pass without thinking, and one that thinks without passing.
\end{enumerate}}
\answer{
\begin{enumerate}
\item The mathematical objection (from G\"odel) and the argument from consciousness remain intact, because neither depends on how good the machine gets.
The arguments from disability (``a machine will never do X'') have gone badly for their proponents one item at a time.
Lady Lovelace's objection, that a machine originates nothing, has aged into the interesting one, because it now reads as a claim about training data instead of a claim about programming.
\item Mostly about the test.
Turing's numbers came out roughly right and arrived late rather than never, but current systems pass casual five-minute questioning while failing in ways he never anticipated, which suggests that the test measures conversational fluency and not thought.
Ask yourselves why the field ever accepted so passable a test as its criterion.
\item A lookup table over every five-minute conversation would pass and think nothing; it is finite, absurdly large, and contains no reasoning.
A chess engine, or any planner in this course, reasons its way to a decision no human could match, and an interrogator would spot it as a machine inside one question.
\end{enumerate}
}

\item 
%% SOURCE: AIMA 3e Ex 1.9, verbatim, with AIMA's answer. Checked against the 3e Instructor's
%% Manual on 2026-09-26; the only changes are the house conversion of em-dashes and quotes.
\question{Why would evolution tend to result in systems that act rationally?
What goals are such systems designed to achieve?}
\answer{Evolution tends to perpetuate organisms (and combinations and mutations of organisms) that are successful enough to reproduce.
That is, evolution favors organisms that can optimize their performance measure to at least survive to the age of sexual maturity, and then be able to win a mate.
Rationality just means optimizing performance measure, so this is in line with evolution.}

\item 
%% SOURCE: AIMA 3e Ex 1.10, verbatim, with AIMA's answer. Checked against the 3e Instructor's
%% Manual on 2026-09-26; the only changes are the house conversion of em-dashes and quotes.
\question{Is AI a science, or is it engineering? Or neither or both? Justify your answer.}
\answer{This question is intended to be about the essential nature of the AI problem and what is required to solve it, but could also be interpreted as a sociological question about the current practice of AI research.
...
Of course, engineers do bits of science (e.g., they measure the properties of building materials) and scientists do bits of engineering to create new devices and so on. 
As described in Section 1.1, the ``human'' side of AI is clearly an empirical science (called cognitive science these days) because it involves psychological experiments designed out to find out how human cognition actually works.
What about the ``rational'' side?
If we view it as studying the abstract relationship among an arbitrary task environment, a computing device, and the program for that computing device that yields the best performance in the task environment, then the rational side of AI is really mathematics and engineering;
it does not require any empirical knowledge about the actual world (and the actual task environment) that we inhabit;
that a given program will do well in a given environment is a theorem (The same is true of pure decision theory). 
In practice, however, we are interested in task environments that do approximate the actual world, so even the rational side of AI involves finding out what the actual world is like. 
For example, in studying rational agents that communicate, we are interested in task environments that contain humans, so we have to find out what human language is like.
In studying perception, we tend to focus on sensors such as cameras that extract useful information from the actual world.
(In a world without light, cameras would not be much use.)
Moreover, to design vision algorithms that are good at extracting information from camera images, we need to understand the actual world that generates those images.
Obtaining the required understanding of scene characteristics, object types, surface markings, and so on is a quite different kind of science from ordinary physics, chemistry, biology, and so on, but it is still science.
...
\item 
%% SOURCE: AIMA 3e Ex 1.11, verbatim, with AIMA's answer. Checked against the 3e Instructor's
%% Manual on 2026-09-26; the only changes are the house conversion of em-dashes and quotes.
%% Ex 1.12 (genes) and 1.13 (atoms) follow it, commented out.
\question{``Surely computers cannot be intelligent -- they can do only what their programmers tell them.''
Is the latter statement true, and does it imply the former?}
\answer{This depends on your definition of ``intelligent'' and ``tell.''
In one sense computers only do what the programmers command them to do, but in another sense what the programmers consciously tells the computer to do often has very little to do with what the computer actually does.
Anyone who has written a program with an ornery bug knows this, as does anyone who has written a successful machine learning program.
So in one sense Samuel ``told'' the computer ``learn to play checkers better than I do, and then play that way,'' but in another sense he told the computer ``follow this learning algorithm'' and it learned to play.
So we are left in the situation where you may or may not consider learning to play checkers to be s sign of intelligence (or you may think that learning to play in the right way requires intelligence, but not in this way), and you may think the intelligence resides in the programmer or in the computer.}
...
\begin{enumerate}
 ...
 \item
%% SOURCE: AIMA 3e Ex 2.3, items (h) and (i).
%% Items (a) to (g) are commented out.
  \question{For each of the following assertions, say whether it is true or false and support your answer with examples or counterexamples where appropriate:
\begin{enumerate}
...
\item False.
Some actions are stupid -- and the agent may know this if it has a model of the environment -- even if one cannot perceive the environment state.
\item False.
Unless it draws the perfect hand, the agent can always lose if an opponent has better cards.
This can happen for game after game.
The correct statement is that the agent’s expected winnings are nonnegative.
  \end{enumerate}}

\item
%% SOURCE: AIMA 3e Ex 2.4, items (c) used AI books and (h) auction, of its eight.
%% The note on the four-versus-six dimensions is ours, 2026-09-08.
...
\answer{\begin{enumerate}
\item Partially observable, deterministic, sequential, static, discrete, single agent.
...
\item
%% SOURCE: AIMA 3e Ex 2.6, items (b) and (e).
%% Items (a), (c) and (d) are commented out.
...
\item Yes; the agent function might specify that the agent print true when the percept is a Turing machine program that halts, and false otherwise.
...
\item It depends on the program and the environment.
If the environment is dynamic, speeding up the machine may mean choosing different (perhaps better) actions and/or acting sooner.
If the environment is static and the program pays no attention to the passage of elapsed time, the agent function is unchanged.
\end{enumerate}
}
...
\begin{enumerate}
\item Murphy's law: twenty-five percent of the time, the Suck action fails to clean the floor if it is dirty and deposits dirt onto the floor if the floor is clean.
How is your agent program affected if the dirt sensor gives the wrong answer 10\% of the time?
\item Small children: At each time step, each clean square has a 10\% chance of becoming dirty.
Can you come up with a rational agent design for this case?
\end{enumerate}
}
\answer{
\begin{enumerate}
\item For a reflex agent, this presents no additional challenge, because the agent will continue to $\mathit{Suck}$ as long as the current location remains dirty.
For an agent that constructs a sequential plan, every $\mathit{Suck}$ action would need to be replaced by ``$\mathit{Suck}$ until clean.''
If the dirt sensor can be wrong on each step, then the agent might want to wait for a few steps to get a more reliable measurement before deciding whether to $\mathit{Suck}$ or move on to a new square.
Obviously, there is a trade-off because waiting too long means that dirt remains on the floor (incurring a penalty), but acting immediately risks either dirtying a clean square or ignoring a dirty square (if the sensor is wrong).
A rational agent must also continue touring and checking the squares in case it missed one on a previous tour (because of bad sensor readings).
It is not immediately obvious how the waiting time at each square should change with each new tour.
These issues can be clarified by experimentation, which may suggest a general trend that can be verified mathematically.
This problem is a partially observable Markov decision process -- see Chapter 17.
Such problems are hard in general, but some special cases may yield to careful analysis.
\item In this case, the agent must keep touring the squares indefinitely.
The probability that a square is dirty increases monotonically with the time since it was last cleaned, so the rational strategy is, roughly speaking, to repeatedly execute the shortest possible tour of all squares.
(We say ``roughly speaking'' because there are complications caused by the fact that the shortest tour may visit some squares twice, depending on the geography.)
...
\item
%% SOURCE: Ours, 2026-09-08.
%% No AIMA counterpart: it compares the Wumpus World of AIMA Ch 7 against the vacuum world of Ch 2 to set up Lectures 5 and 6.
...
\item A vacuum agent can decide what to do from what it senses now; a Wumpus agent cannot.
Account for that using the classification you just gave.
...
\answer{
\begin{enumerate}
\item Both are discrete and static, and both are deterministic.
They part company on observability.
The vacuum world as Lecture 2 formulates it is fully observable, so the agent knows which rooms are dirty.
The Wumpus World is partially observable: the agent perceives a breeze, a stench or a glitter in the square it occupies, and never sees a pit or the Wumpus directly.
The uncertainty here springs from the agent's ignorance and not from chance, and that is the reason logic answers it where probability would be the wrong tool.
\item Each percept in the Wumpus World is a consequence of facts about neighbouring squares, so a breeze tells the agent that some adjacent square holds a pit without saying which.
To act, the agent must combine percepts gathered in different squares at different times and work out what follows from them together, and that demands a representation of what the agent knows, where a rule from percept to action would not do.
Lectures 5 and 6 build that machinery.
\item Moving into a square that holds a pit or the live Wumpus ends the run, so the agent can never recover from a wrong inference there.
A guessing agent loses more than time, it loses the game, and so the agent should enter only those squares it can prove safe.
``I have no evidence of a pit here'' and ``I can prove there is no pit here'' make different claims, and only the second justifies the move.
\end{enumerate}
}
