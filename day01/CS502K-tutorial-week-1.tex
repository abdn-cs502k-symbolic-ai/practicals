...
far from those normally carried out in human cognition.
  \item Yes. The task is genuinely uncertain, the filter learns its decision from data rather than from a rule someone wrote, and an adversary changes the distribution deliberately. A filter built from a fixed list of banned words would not qualify on the same reading, which shows that the question is about how the system decides, not about what it decides.
  \item Yes, on any of the four definitions in Section 1.1, but it is worth asking which one. It acts humanly, which is exactly what Turing proposed to measure. It is not built to think rationally: no part of it manipulates sentences that stand for facts, which is the tradition this course follows. Note also that it holds no explicit goal and optimises no stated utility, so the ``acting rationally'' definition fits it only loosely. A system can be uncontroversially AI and still sit outside the strand we study.
  \end{enumerate}
}
...
\answer{
\begin{enumerate}
\item The mathematical objection (from G\"odel) and the argument from consciousness survive largely intact, because neither turns on how good the machine gets. The arguments from disability (``a machine will never do X'') have gone badly for their proponents one item at a time. Lady Lovelace's objection, that a machine originates nothing, is the interesting case: it now reads as a claim about training data rather than about programming.
\item Mostly about the test. Turing's numbers were roughly right and arrived late rather than never, but current systems pass casual five-minute questioning while failing in ways he did not anticipate, which suggests the test measures fluency in conversation rather than thought. That an easily passed test was ever taken as the criterion is the point worth discussing.
\item A lookup table over every five-minute conversation would pass and think nothing; it is finite, absurdly large, and contains no reasoning. A chess engine, or any planner in this course, reasons its way to a decision no human could match and would be identified as a machine within one question.
\end{enumerate}
}
