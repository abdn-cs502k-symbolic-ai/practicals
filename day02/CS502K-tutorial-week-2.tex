...
\item \question{Familiarise yourself with the search algorithms in the aima-python library (\url{https://github.com/aimacode/aima-python}). You find two versions of Jupyter notebooks with search algorithms: (1) \texttt{search.ipynb} (corresponding to AIMA 3e) and (2) \texttt{search4e.ipynb} (corresponding to AIMA 4e). Although in this course we largely follow AIMA 3e, for searching algorithms it is best you learn from both versions as each has a different set of examples. Consider the following tasks:
\begin{enumerate}
\item Inspect the code for the abstract Problem and Node classes which are defined in the \texttt{search.py} library.
...
\item Inspect the code corresponding to the 8 Puzzle, focusing mainly on how you define the problem, define cost and heuristic functions to search problems. Investigate the search space explored (nodes expanded) and the paths to solutions.
\item Model the problem of the jugs introduced above, and use the algorithms to find a solution. (Note: a solution to the Jugs problem is in \texttt{search4e.ipynb}.)
\end{enumerate}
}
