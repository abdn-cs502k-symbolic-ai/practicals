...
Follows Lecture 3 (Uninformed Search) and Lecture 4 (Informed Search), and runs over both practical sessions of its week.
...
- Implement those algorithms against the `Problem` interface from Tutorial 1
...
Tutorial 1 gave students the `Problem` class and no search algorithm.
...
Lecture 4 contradicts itself on two heuristic values, and the tutorial follows the table.
The `h_SLD` table printed beside the Romania map gives Fagaras 178 and Pitesti 98, which are the AIMA 2e values.
...
A language model writes every algorithm in this tutorial correctly and in seconds.
We know, and we set the exercises anyway, because the hand trace and the code have to agree, and you cannot reconcile them without understanding either.
