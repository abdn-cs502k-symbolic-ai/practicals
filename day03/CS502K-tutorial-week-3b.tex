...
%% SOURCE: AIMA 3e Ex 9.9, question verbatim. Checked against the 3e Instructor's Manual on
%% 2026-09-26, which also corrected the number: this was labelled id=9.11 after the online
%% aima-exercises set, where 9.11 is a different exercise, and 3e Ex 9.11 is the database indexing
%% question. The answer is AIMA's proof, with its goals and unifiers unchanged, but we set it as a
%% table and added the closing paragraph comparing the two searches, so it is not verbatim.
%% Item (b) is AIMA's second half, restored 2026-09-15: L06 teaches the forward-chaining algorithm
%% and nothing in this tutorial exercised it.
\item\label{q:axioms}
...
\answer{
\begin{enumerate}
\item $\exists x \forall y\, [M(x, y) \Leftrightarrow \lnot M(y, y)]$, that is \[\exists x \forall y\, ([M(x, y) \Rightarrow \lnot M(y, y)] \land [\lnot M(y, y) \Rightarrow M(x, y)])\]
...
\item Applying factoring to $C_1$ with the substitution $\{y/\mi{Sk}_1\}$ gives \[C_3: \lnot M(\mi{Sk}_1, \mi{Sk}_1)\]
Applying factoring to $C_2$ with the substitution $\{y/\mi{Sk}_1\}$ gives \[C_4: M(\mi{Sk}_1, \mi{Sk}_1)\]
