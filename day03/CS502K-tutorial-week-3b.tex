...
Question 8 sends you to the notebook \texttt{tutorial3-logic.ipynb} in this folder, the same one the first session uses.

\textbf{Before this session, work through the bonus deck.}
Lecture 6 has no room for Skolemisation, clausal form, or resolution lifted by unification, and questions 3(f), 4(b), 6 and 7 below all depend on them.
They are in \emph{Bonus: Clausal Form and Resolution in FOL} (\texttt{ai-lecture06-bonus-slides-clausal-form.pdf} in the lecture repository), which is self-study and takes about ten minutes.
It assumes the bonus deck from the first session, so read that one first if you have not.
...
%% SOURCE: Ours (inherited stem). Depends on the resolution frames of both bonus decks, written 2026-09-15.
\item
...
Resolution only ever does one thing, which is to refute a set of clauses, and validity and unsatisfiability are two different sets to hand it.}
...
\begin{enumerate}
\item Implement \texttt{unify}, including the occurs check, and check it against the four pairs of question 2.
\item Make the occurs check switchable, then show that turning it off lets the notebook derive $\exists z\, P(z, z)$ from $\forall x \exists y\, P(x, y)$, which is the failure of question 4(b).
\item Implement \texttt{fol\_bc\_ask}, backward chaining with unification, and run it on the bird knowledge base you wrote in question 3.
\end{enumerate}}
