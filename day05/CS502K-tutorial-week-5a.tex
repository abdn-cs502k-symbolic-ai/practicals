...
This tutorial covers Lecture 9, and is the first practical session of week 13.
...
\textbf{Assessment 3 is due on Thursday of this week}, and it gives a fifth of its marks for explaining why your benchmark numbers turn where they do, in terms of your domain and the heuristic you ran.
This session is where that explanation is built.
Questions 1 to 4 are the machinery, on a problem small enough to do by hand; questions 5 and 6 apply it, first to a domain you have already written and then to your own.

\textbf{Before you start.}
You need Fast Downward, which you built in question 6 of Tutorial 4.
You also need the two-gripper blocks world you wrote for question 5 of that tutorial.
If you did not finish it, take the reference version from the Tutorial 4 solutions archive and use that instead; nothing here depends on your having written it yourself.

\textbf{The files.}
Questions 1 to 4 use \texttt{gripper/}, in \texttt{tutorial-week-5-files.zip}, released with this tutorial.

\textbf{A note on notation.}
The lecture writes $h^{max}$, $h^{add}$ and $h^{FF}$; Fast Downward calls the same three \texttt{hmax()}, \texttt{add()} and \texttt{ff()}.
...
A language model will compute these heuristic values for you, and will get them right.
...
The instance is smaller than the one in Tutorial 4: two rooms \texttt{rooma} and \texttt{roomb}, two balls \texttt{ball1} and \texttt{ball2}, and a single gripper \texttt{left}.
...
\item Give an optimal plan for the real task, and its length. This is $h^{*}$.
...
\item In which layer does each goal fact first appear, and what is $h^{max}$? Check it against your answer to question 2.
...
\item Both goal facts first appear in $F_2$, so $h^{max} = 2$, which agrees with question 2.
...
That is 5 actions, so $h^{FF} = 5$, and it is the relaxed plan you wrote by inspection in question 1(c).
...
\item Each run also reports the states it expanded. Three runs, one problem, and the plan is the same every time: what are the expansion counts telling you that the plan is not?
...
Question 5 is the same comparison on a problem large enough for the differences to mean something.
...
This is why optimal planning gets hard so fast, and why almost everything used in practice, including the configuration you will run in question 6, gives up optimality first and asks questions later.
\end{enumerate}}
...
\item
\question{Everything so far has been one problem, small enough to hold in your head, and question 4(b) showed that a problem that small cannot tell two heuristics apart.
...
Take your two-gripper blocks world from question 5 of Tutorial 4 and run Fast Downward on it three times:}
...
\item Questions 1 to 3 computed $h^{FF}$ by throwing away delete effects. Blocks world is the standard example of a domain where that goes badly wrong. Which delete effect is being ignored, and how does ignoring it produce the plan costs you measured?
\end{enumerate}}
...
What a good answer looks like is question 5(d) and 5(e) applied to your own model: a claim about your domain, supported by two numbers from your table, in terms the lecture gave you.
...
This tutorial had the planner work out its own guidance by relaxing your domain; Tutorial 5b hands the planner the guidance instead, written by you as a hierarchy of methods, and asks what each approach buys.

\textbf{Credits.}
The gripper domain is the one from Lecture 7, and is originally from the AIPS-98 planning competition.
The delete relaxation and the two heuristics of question 2 are due to Bonet and Geffner; the relaxed planning graph is Blum and Furst's, by way of Hoffmann; and $h^{FF}$ is Hoffmann and Nebel's.
